...
/**
 * Renders alpha Flexible Content
 *
 * @param  string $template The alpha Flexible template
 */
...
/**
 * echo alpha field with HTML Markup
 *
 * @param  string $name         alpha Object
 ...
 * @return Mixed                render alpha Object
 */
...
/**
 * Prints an anchor tag from an array of alpha Link object
 */
function ja_the_link($link = array(),  $classes = '', $target = '', $before = '', $after = '', $attributes = '')
{
  ...
  if (!strpos($attributes, 'aria-label')) {
    $attributes .= ' aria-label="' . wp_sprintf(__('Open for %s', 'alpha'), strip_tags($link['url'])) . '"';
  }
...
?>
    <div class="iframe-container <?php echo $custom_classes; ?>">
      <iframe width="100%" src="https://www.youtube.com/embed/<?php echo $youtube_vars['v'] . '?' . $video_attributes; ?>" title="<?php echo wp_sprintf(__('Youtube Video ID: %s', 'alpha'), $youtube_vars['v']); ?>" allow="encrypted-media" allowfullscreen></iframe>
    </div>
  <?php
  ...
  ?>
    <div class="iframe-container <?php echo $custom_classes; ?>">
      <iframe width="100%" src="https://player.vimeo.com/video/<?php echo $vimeo_id . '?' . $video_attributes; ?>" title="<?php echo wp_sprintf(__('Vimeo ID: %s', 'alpha'), $vimeo_id); ?>" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
    </div>
  <?php else : ?>
...
function asset_path($path, $echo = false)
{
	...
	$base_path = get_template_directory_uri() . '/dist';
...
function menu_item_class( $classes, $item ) {
  $classes[] = "navbar__links-link"; // you can add multiple classes here
  return $classes;
}

add_filter( 'nav_menu_css_class' , 'menu_item_class' , 10, 2 );

/* Add custom class to link (a tag) in menu */

function _namespace_modify_menuclass($ulclass) {
  return preg_replace('/<a /', '<a class="secondary-body-text"', $ulclass);
}
